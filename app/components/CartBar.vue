...
<template>
  <div v-if="cart.items.length > 0"
    class="fixed bottom-8 left-0 right-0 z-50 px-4 flex justify-center pointer-events-none">
    <div
      class="w-full max-w-2xl bg-[var(--bg-card)] rounded-[2.5rem] shadow-[0_0_40px_rgba(0,255,255,0.5)] flex items-stretch overflow-hidden h-[4.5rem] border-[3px] border-[var(--border)] pointer-events-auto">
      <div class="flex-1 px-6 flex items-center">
        <input v-model="cart.guestName" placeholder="Ваше ім'я..."
          class="bg-transparent text-[var(--text-main)] font-black placeholder:text-[var(--text-muted)] outline-none w-full text-xl h-full" />
      </div>
      <button @click="$emit('placeOrder')" :disabled="cart.cooldown > 0"
